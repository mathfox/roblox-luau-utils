...
function Error.new(options: { trace: string?, context: string, kind: string }, parent: Error?)
	return setmetatable({
...
function Error.isKind(object, kind: string)
	return type(object) == "table" and getmetatable(object) == Error and object.kind == kind
end
...
-- Calls an `Promise` executor with error handling.
local runExecutor
...
function Promise._new<V...>(traceback: string, executor: Executor<V...>, parent: Promise<...any>?): Promise<V...>
	local self = setmetatable({
		...
		-- Lua doesn't like sparse arrays very much, so we explicitly store the length of _values to handle middle nils.
		_valuesLength = -1,
		...
		-- The "parent" of this Promise in a promises chain. Required for cancellation propagation upstream.
		_parent = parent,

		-- Consumers are Promises that have chained onto this one. We track them for cancellation propagation downstream.
...
function Promise._all<T>(traceback: string, promises: Array<Promise<T>>, amount: number?): Promise<Array<T>>
	...
	return if #promises == 0 or amount == 0
		then Promise.resolve({})
			...
			Promise._new(traceback, function(resolve, reject, onCancel)
				...
				local function resolveOne(i, firstReturnValue)
					if done then
						return
					end
					...
					if amount == nil then
						resolvedValues[i] = firstReturnValue
					else
						resolvedValues[resolvedCount] = firstReturnValue
					end

					-- Check if the condition for resolving the result Promise is pleased.
				...
				for i, promise in ipairs(promises) do
					newPromises[i] = promise:andThen(function(firstReturnValue)
						...
						-- Check whenever requested amount of resolved promises can not be reached so we should cancel all pending promises and return a rejection from a latest rejected Promise.
				...
				-- Check if the enough amount of Promises was already resolved while andThen handlers was attached.
				if done then
					cancel()
				end
			end)
...
function Promise.allSettled(promises: Array<Promise<...any>>): Promise<Array<EnumeratorItem<string>>>
	...
	return if #promises == 0
		then Promise.resolve({})
			...
			Promise._new(debug.traceback(nil, 2), function(resolve, _, onCancel)
				-- An array to contain our resolved values from the given promises.
				local fates = {}
				local newPromises = {}
...
do
	...
	local function createAdvancer(traceback: string, callback, resolve, reject)
		return function(...)
			local ok, result, resultLength = runExecutor(traceback, callback, ...)
	...
	function Promise.prototype:_andThen(traceback: string, successHandler, failureHandler)
		...
		return Promise._new(traceback, function(resolve, reject, onCancel)
			...
			local successCallback, failureCallback =
				if successHandler then createAdvancer(traceback, successHandler, resolve, reject) else resolve,
				if failureHandler then createAdvancer(traceback, failureHandler, resolve, reject) else reject

			if self._status == Promise.Status.Started then
				-- If we haven't finished yet, put ourselves into the queue
				table.insert(self._queuedResolve, successCallback)
				table.insert(self._queuedReject, failureCallback)

				onCancel(function()
					-- These are guaranteed to exist because the cancellation handler is guaranteed to only be called at most once
					if self._status == Promise.Status.Started then
...
function Promise.prototype:catch<R...>(failureHandler: (...any) -> R...): Promise<R...>
...
function Promise.prototype:tap(tapHandler: (...any) -> ...any)
...
function Promise.prototype:_consumerCancelled(consumer: Promise<...any>)
	if self._status ~= Promise.Status.Started then
		return
	end
...
function Promise.prototype:_finally(traceback: string, finallyHandler: (...any) -> ...any)
	self._unhandledRejection = false
	...
	return Promise._new(traceback, function(resolve, reject, onCancel)
		...
		local finallyCallback = if finallyHandler
			then function(...)
...
function Promise.prototype:awaitStatus(): (string, ...any)
	self._unhandledRejection = false
...
function Promise.onUnhandledRejection(callback: (Promise<...any>, ...any) -> ())
...
return Promise :: {
	Error: {
		isKind: (object: any, kind: string) -> boolean,
		Kind: {
			ExecutionError: string,
			AlreadyCancelled: string,
			NotResolvedInTime: string,
			TimedOut: string,
		},
	},
	Status: {
